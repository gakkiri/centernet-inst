...
class CenterNetGT(object):
    ...
    def generate(config, batched_input):
        ...
        for data in batched_input:
            ...
            ## MASK
            if len(boxes) == 0:
                seg_mask_list.append(torch.tensor([]))
                continue
            gt_masks = bbox_dict['gt_masks'].tensor.float()
            gt_masks = interpolate(gt_masks.unsqueeze(0), scale_factor=1/4, mode='bilinear')
            seg_mask_list.append(gt_masks[0].gt(0).float())
