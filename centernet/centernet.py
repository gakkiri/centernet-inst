...
from .losses import reg_l1_loss, modified_focal_loss, DiceLoss
...
class CenterNet(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.seg_weight = cfg.MODEL.CENTERNET.SEG_WEIGHT
        self.mask_loss_func = DiceLoss(cfg.MODEL.CENTERNET.SEG_FEAT_CHANNEL)
    ...
    def losses(self, pred_dict, gt_dict):
        ...
        for k in gt_dict:
            if isinstance(gt_dict[k], torch.Tensor):
                ...
                for i in range(len(gt_dict[k])):
                    gt_dict[k][i] = gt_dict[k][i].to(cur_device)
        ...
        ### MASK
        loss_mask = self.mask_loss_func(pred_dict['seg_feat'], pred_dict['conv_weight'],
                                        gt_dict['reg_mask'], index, gt_dict['gt_masks'])

        loss = {"loss_cls": loss_cls, "loss_box_wh": loss_wh, "loss_center_reg": loss_reg,
                'loss_mask': loss_mask * self.seg_weight}
        return loss
    ...
    def inference(self, images):
        ...
        down_masks = results['pred_masks'][None, ...]
        masks = F.interpolate(down_masks, size=(int(ori_h), int(ori_w)), mode='bilinear', align_corners=False).gt(0.5).squeeze(0)
    ...
    def decode_prediction(self, pred_dict, img_info):
        ...
        seg_feat = pred_dict['seg_feat']
        conv_weight = pred_dict['conv_weight']

        boxes, scores, classes, mask = \
            CenterNetDecoder.decode(fmap, wh, seg_feat, conv_weight, reg)
        ...
        raw_w, raw_h = img_info['size']
        # mask = torch.nn.functional.interpolate(mask[None, ...], size=(int(raw_h), int(raw_w)), mode='bilinear').squeeze(0)
        return dict(pred_boxes=boxes, scores=scores, pred_classes=classes, pred_masks=mask)
